...
class BookService {
    constructor() {
        this.books = randomBooks.map(book => {
            // random rating
            book.rating = Number((Math.random() * (10 - 1) + 1).toFixed(2));
            book.stores = [];
            if (book.rating < 5)
                book.stores.push({
                    href: book.thumbnailUrl,
                    type: "ibooks"
                });
            if (book.rating > 5)
                book.stores.push({
            ...
            book.year = new Date(book.publishedDate.$date).getFullYear();
            book.author = book.authors[0];
            book.longDescription = book.longDescription || "";
            if(book.longDescription.trim() === "") book.longDescription = randomBooks[0].longDescription;
            return book;
        });
    }
    paginate(perPage = 5, pageNo = 1) {
        --pageNo;
        ...
        return {
            ...
            totalPages: Math.ceil(this.books.length / perPage)
        };
    }
}
